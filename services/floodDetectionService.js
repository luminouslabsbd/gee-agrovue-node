...
class FloodDetectionService {
  ...
  async detectFloods(fieldBoundary, fieldId, currentDate = null) {
    try {
      ...
      const analysisDate = currentDate || new Date().toISOString().split('T')[0];
      
      console.log(`🌊 Analyzing flood status for field ${fieldId} on ${analysisDate}`);
      
      // 1. Detect current flood status
      const currentFloodStatus = await this._detectCurrentFlood(geometry, analysisDate);
      
      // 2. Analyze historical floods
      const historicalFloods = await this._analyzeHistoricalFloods(geometry, analysisDate);
      
      // 3. Calculate flood risk
      const floodRisk = this._calculateFloodRisk(historicalFloods);
      
      // 4. Get water extent statistics
      const waterExtent = await this._calculateWaterExtent(geometry, analysisDate);
   ...
   */
  async generateFloodTimeSeries(fieldBoundary, fieldId, startDate, endDate, intervalDays = 30) {
    try {
      const geometry = this._geoJsonToEEGeometry(fieldBoundary);
      ...
      for (const date of dates) {
        ...
        const waterData = await this._getWaterExtentForPeriod(geometry, date, nextDate);
  ...
  /**
   * Detect current flood status
   * @private
   */
  async _detectCurrentFlood(geometry, date) {
    try {
      // Get recent SAR imagery (last 12 days)
      const endDate = date;
      const startDate = this._subtractDays(date, 12);
      
      const collection = this.ee.ImageCollection(this.SENTINEL1_DATASET)
        .filterBounds(geometry)
        .filterDate(startDate, endDate)
        .filter(this.ee.Filter.listContains('transmitterReceiverPolarisation', 'VV'))
      ...
      if (size === 0) {
        return {
          ...
          confidence: 'low',
          message: 'No recent SAR imagery available',
          water_percentage: 0,
    ...
    try {
      ...
      for (let i = 0; i < size; i += sampleInterval) {
        try {
          ...
          const imageDate = this.ee.Date(image.get('system:time_start')).format('YYYY-MM-dd').getInfo();
  ...
  _calculateFloodRisk(historicalFloods) {
    ...
    const avgWaterPercentage = historicalFloods.reduce((sum, f) => sum + f.water_percentage, 0) / floodCount;
  ...
  _calculateFloodStatistics(timeSeriesData) {
    ...
    return {
      ...
      flood_frequency_percentage: parseFloat(((floodEvents.length / timeSeriesData.length) * 100).toFixed(2)),
      max_water_percentage: parseFloat(Math.max(...waterPercentages).toFixed(2)),
      avg_water_percentage: parseFloat((waterPercentages.reduce((a, b) => a + b, 0) / waterPercentages.length).toFixed(2)),
  ...
  _geoJsonToEEGeometry(geoJson) {
    ...
    throw new Error('Unsupported geometry type. Only Polygon and MultiPolygon are supported.');
  }
