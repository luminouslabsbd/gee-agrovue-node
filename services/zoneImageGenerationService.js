...
class ZoneImageGenerationService {
  constructor(ee) {
    ...
    this.ZONE_THRESHOLDS = {
      excellent: { min: 0.7, max: 1.0, color: '#006400', label: 'Excellent', rgb: [0, 100, 0] },
      good: { min: 0.5, max: 0.7, color: '#32CD32', label: 'Good', rgb: [50, 205, 50] },
      moderate: { min: 0.3, max: 0.5, color: '#FFD700', label: 'Moderate', rgb: [255, 215, 0] },
      poor: { min: 0.1, max: 0.3, color: '#FF8C00', label: 'Poor', rgb: [255, 140, 0] },
      very_poor: { min: -0.2, max: 0.1, color: '#DC143C', label: 'Very Poor', rgb: [220, 20, 60] }
    };
   ...
   */
  async generateZoneImage(fieldBoundary, fieldId, date, gridSize = 50, cropType = 'rice') {
    try {
      ...
      const imageMetadata = await this._storeImageMetadata(
        imageId, fieldId, date, zoneAnalysis, yieldData, meanNDVI, stdNDVI, minNDVI, maxNDVI
      );
      
      // Calculate productivity status
      const productivityStatus = this._calculateProductivityStatus(meanNDVI, zoneAnalysis, yieldData);
      ...
      return {
        ...
        images: {
          ndvi_image_url: `/zone-images/${imageId}/ndvi_map.png`,
          zone_image_url: `/zone-images/${imageId}/zone_map.png`,
          metadata_url: `/zone-images/${imageId}/metadata.json`,
          legend_url: `/zone-images/${imageId}/legend.json`
        },
  ...
  async _analyzeZones(ndvi, zones, fieldGeometry) {
    ...
    for (const zone of zones) {
      try {
        ...
        if (area > 0.001) { // Only include zones with significant area
        ...
        console.warn(`Warning: Could not analyze zone ${zone.zone_id}:`, error.message);
      }
    }
  ...
  _classifyZone(meanNDVI) {
    if (meanNDVI >= 0.7) {
      return { code: 'excellent', label: 'Excellent', color: '#006400', priority: 1 };
    } else if (meanNDVI >= 0.5) {
      return { code: 'good', label: 'Good', color: '#32CD32', priority: 2 };
    } else if (meanNDVI >= 0.3) {
      return { code: 'moderate', label: 'Moderate', color: '#FFD700', priority: 3 };
    } else if (meanNDVI >= 0.1) {
      return { code: 'poor', label: 'Poor', color: '#FF8C00', priority: 4 };
    } else {
      return { code: 'very_poor', label: 'Very Poor', color: '#DC143C', priority: 5 };
    }
  }
  ...
  _calculateZoneSummary(zones) {
    ...
    zones.forEach(zone => {
      ...
      summary.by_classification[zone.classification_code].total_area += zone.area_hectares;
    ...
    Object.keys(summary.by_classification).forEach(key => {
      ...
      classification.percentage = parseFloat(((classification.total_area / summary.total_area) * 100).toFixed(2));
    });
  ...
  _calculateYieldPredictions(zoneAnalysis, cropType) {
    ...
    const zoneYields = zones.map(zone => {
      ...
      return {
        ...
        total_yield_tons: parseFloat((estimatedYield * zone.area_hectares).toFixed(2)),
        yield_quality: this._classifyYieldQuality(estimatedYield, yieldFactor.max_yield),
        classification: zone.classification
      };
    });

    // Calculate field-level yield
    const totalYield = zoneYields.reduce((sum, z) => sum + z.total_yield_tons, 0);
    ...
    return {
      ...
      field_summary: {
        ...
        potential_max_yield_tons: parseFloat((yieldFactor.max_yield * totalArea).toFixed(2)),
        yield_efficiency_percentage: parseFloat(((averageYield / yieldFactor.max_yield) * 100).toFixed(2))
      }
    };
  }
  ...
  _calculateProductivityStatus(meanNDVI, zoneAnalysis, yieldData) {
    ...
    const uniformityScore = 100 - (summary.by_classification.poor?.percentage || 0) - (summary.by_classification.very_poor?.percentage || 0);

    const overallScore = (ndviScore * 0.4 + yieldScore * 0.4 + uniformityScore * 0.2);
    ...
    return {
      ...
      zone_distribution: {
        excellent_percentage: summary.by_classification.excellent?.percentage || 0,
        good_percentage: summary.by_classification.good?.percentage || 0,
        moderate_percentage: summary.by_classification.moderate?.percentage || 0,
        poor_percentage: summary.by_classification.poor?.percentage || 0,
        very_poor_percentage: summary.by_classification.very_poor?.percentage || 0
      },
   ...
   */
  async _storeImageMetadata(imageId, fieldId, date, zoneAnalysis, yieldData, meanNDVI, stdNDVI, minNDVI, maxNDVI) {
    const imageDir = path.join(this.STORAGE_DIR, imageId);
    ...
    const metadata = {
      ...
      images: {
        ndvi_image: `/zone-images/${imageId}/ndvi_map.png`,
        zone_image: `/zone-images/${imageId}/zone_map.png`
      },
  ...
  _createLegend() {
    return {
      ...
      zones: [
        { label: 'Excellent', ndvi_range: '0.7 - 1.0', color: '#006400', description: 'Optimal crop health and productivity' },
        { label: 'Good', ndvi_range: '0.5 - 0.7', color: '#32CD32', description: 'Healthy vegetation, good productivity' },
        { label: 'Moderate', ndvi_range: '0.3 - 0.5', color: '#FFD700', description: 'Moderate health, needs monitoring' },
        { label: 'Poor', ndvi_range: '0.1 - 0.3', color: '#FF8C00', description: 'Poor health, intervention needed' },
        { label: 'Very Poor', ndvi_range: '< 0.1', color: '#DC143C', description: 'Critical condition, urgent action required' }
      ]
    };
  }

  /**
   * Update index of all zone images
   */
  _updateIndex(imageId, fieldId, date) {
    ...
    if (index.images.length > 100) {
      const removed = index.images.shift();
      const removedDir = path.join(this.STORAGE_DIR, removed.image_id);
